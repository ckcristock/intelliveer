...
export class ProviderFormComponent implements OnInit {
	...
	initForm(data?: any) {
		...
		this.Form = this.fb.group({
			...
			degree: [data?.degree || ''],
			officeName: [data?.officeName || '', Validators.required],
	...
	officeNameValid() {
		return this.Form.get('officeName')?.valid;
	}

	pPhoneNumberValid() {
		return (
			this.Form.get('pPhoneNumber')?.value.length > 0 &&
			this.Form.get('pPhoneNumber')?.valid
		);
	}
