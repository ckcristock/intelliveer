...
@Component({
  selector: 'app-edit-referer',
  templateUrl: './edit-referer.component.html',
  styleUrls: ['./edit-referer.component.scss'],
})
export class EditRefererComponent implements OnInit, OnDestroy {
  id: string | undefined;
  data: any;
  ...
  update(data: any) {
    if (this.id && this.selectedBusinessGroup) {
      this.practiceLocation
        ...
        .subscribe({
          next: (res) => {
            this.router.navigate(['/dashboard/patient/patient-user/referer']);
          },
          error: () => {},
        });
    }
