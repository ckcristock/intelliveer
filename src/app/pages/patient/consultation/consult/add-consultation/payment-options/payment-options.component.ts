...
export class PaymentOptionsComponent implements OnInit {
	...
	totalDownTable(event: any, step: any) {
		...
		let totalDPBG =
			((this.downPayment + this.downPayment2 - this.maxDownPayment1) /
				(this.totalAmount - this.maxDownPayment1)) *
			100;
		this.totalDPBG =
			'linear-gradient(to right, #E6BD5C, #ff4500 ' +
			totalDPBG +
	...
	calculateTotalDowntable(dP1: any, dP2: any, data: any, type?: any) {
		...
		if (type == 'DP1') {
			...
			if (diffData < 25) {
				...
				let DP1: any =
					(this.ratioDownPayment / this.ratioTotal) * DPTotal;
				let DP2: any =
					(this.ratioDownPayment2 / this.ratioTotal) * DPTotal;
	...
	noOfPaymentValue(event: any) {
		...
		if (this.emi == 0 && this.isDownPayment2 == false) {
			...
			if (this.month == 0) {
				...
				data =
					parseFloat(downpayment) / 2 + parseFloat(downPayment2) / 2;
			}
			this.maxDownPayment = data;
			this.downPaymentOneVal = this.downPayment;
			this.downPaymentOneMax = data;
		...
		if (
			...
			if (parseFloat(emi) < parseFloat(this.minPaymentAmountEmi)) {
				...
				if (pmEmi > parseFloat(this.minPaymentAmountEmi)) {
					...
					console.log(monthValue);
					let percentageMonth =
						((monthValue - 1) / (this.totalMonth - 1)) * 100;
					this.sliderBGMonth =
						'linear-gradient(to right, #ECF7F3, #66C293 ' +
						percentageMonth +
						'%, #ECF7F3 ' +
	...
	calculateValueMonthly(event: any) {
		...
		let percentageMonth2 =
			((pmEmi - 25) / (this.maxPaymentAmountEmi - 25)) * 100;
	...
	/** New Calculator */
	addMorePaymentParty() {
		if (this.paymentParty.length == 1) {
			const divisionData = 100 / 2;
			this.paymentPartyPercent[0] = divisionData.toFixed(2);
			this.paymentPartyPercent[1] = divisionData.toFixed(2);
			const totalAmount = this.totalAmount / 2;
			this.paymentPartyTotal[0] = totalAmount;
			this.paymentPartyTotal[1] = totalAmount;
		} else if (this.paymentParty.length == 2) {
			const divisionData = 100 / 3;
			this.paymentPartyPercent[0] = divisionData.toFixed(2);
			this.paymentPartyPercent[1] = divisionData.toFixed(2);
			this.paymentPartyPercent[2] = divisionData.toFixed(2);
			const totalAmount = this.totalAmount / 3;
			this.paymentPartyTotal[0] = totalAmount.toFixed(2);
			this.paymentPartyTotal[1] = totalAmount.toFixed(2);
			this.paymentPartyTotal[2] = totalAmount.toFixed(2);
		}
		this.paymentParty.push({ count: this.paymentParty.length + 1 });
	}
	customPaymentParty(event: any, index: any) {
		let data = event.target.value;
		let count = this.paymentParty.length;
		switch (count) {
			case 1:
				break;
			case 2:
				if (index == 0) {
					const percentData = 100 - parseFloat(data);
					this.paymentPartyPercent[1] = percentData;
					console.log((parseFloat(data) / 100) * this.totalAmount);
					this.paymentPartyTotal[0] = (
						(parseFloat(data) / 100) *
						this.totalAmount
					).toFixed(2);
					this.paymentPartyTotal[1] = (
						(percentData / 100) *
						this.totalAmount
					).toFixed(2);
				} else if (index == 1) {
					const percentData = 100 - parseFloat(data);
					this.paymentPartyPercent[0] = percentData;
					this.paymentPartyTotal[0] = (
						(percentData / 100) *
						this.totalAmount
					).toFixed(2);
			...
			case 3:
				...
				if (index == 0) {
					const pData: any =
						parseFloat(this.paymentPartyPercent[1]) +
						parseFloat(data);
					const pData2 = 100 - parseFloat(pData);
					const pData1 = parseFloat(this.paymentPartyPercent[1]);
					this.paymentPartyPercent[2] = pData2;
					this.paymentPartyTotal[0] = (
						(parseFloat(data) / 100) *
						this.totalAmount
					).toFixed(2);
					...
					this.paymentPartyTotal[2] = (
						(pData2 / 100) *
						this.totalAmount
					).toFixed(2);
				} else if (index == 1) {
					const pData: any =
						parseFloat(this.paymentPartyPercent[0]) +
						parseFloat(data);
					const pData2 = 100 - parseFloat(pData);
					const pData0 = parseFloat(this.paymentPartyPercent[0]);
					this.paymentPartyPercent[2] = pData2;
					this.paymentPartyTotal[0] = (
						(pData0 / 100) *
						this.totalAmount
	...
	calculatePaymentParty() {
		(this.paymentPartyPercent = [100]),
			(this.paymentPartyTotal = [this.totalAmount]);
	}
	paymentPartyPrice(event: any, index: any) {
		let data = event.target.value;
		let count = this.paymentParty.length;
		if (!data) {
			data = 0;
		...
		switch (count) {
			...
			case 2:
				if (index == 0) {
					const pAmountData = this.totalAmount - parseFloat(data);
					this.paymentPartyTotal[1] = pAmountData.toFixed(2);
					this.paymentPartyPercent[1] = (
						(pAmountData * 100) /
						this.totalAmount
					).toFixed(2);
					this.paymentPartyPercent[0] = (
						(data * 100) /
						this.totalAmount
					).toFixed(2);
					console.log(((data * 100) / this.totalAmount).toFixed(2));
				} else if (index == 1) {
					const pAmountData = this.totalAmount - parseFloat(data);
					this.paymentPartyTotal[0] = pAmountData.toFixed(2);
					this.paymentPartyPercent[0] = (
						(pAmountData * 100) /
						this.totalAmount
					).toFixed(2);
			...
			case 3:
				if (index == 0) {
					const pAmountData =
						parseFloat(this.paymentPartyTotal[1]) +
						parseFloat(data);
					const pAmountData1 = this.totalAmount - pAmountData;
					this.paymentPartyTotal[2] = pAmountData1.toFixed(2);
					this.paymentPartyPercent[2] = (
						(pAmountData1 * 100) /
						this.totalAmount
					).toFixed(2);
					this.paymentPartyPercent[1] = (
					...
					const pAmountData =
						parseFloat(this.paymentPartyTotal[0]) +
						parseFloat(data);
					const pAmountData1 = this.totalAmount - pAmountData;
					this.paymentPartyTotal[2] = pAmountData1.toFixed(2);
					this.paymentPartyPercent[2] = (
		...
		if (
			...
			if (parseFloat(emi) < parseFloat(this.minPaymentAmountEmi)) {
				...
				let checkDifference: any =
					this.totalAmount - (this.lastPayment + this.downPayment);
				let number: any = 1;
	...
	calculateNoOfPaymentValue(event: any) {
		...
		if (emi > this.minPaymentAmountEmi) {
			...
			if (this.minDownPayment < this.downPayment) {
				...
				this.downPayment = this.totalAmount - this.emi * this.month;
