...
export class TreatmentComponent implements OnInit {
	...
	sectionMenu(Obj?: any) {
		if (Obj) {
			...
			this.router.navigate(['/dashboard/patient/consultation/consultation/add/treatment']);
	...
	setValueAccordingToOptions(Obj: any) {
		let compareAllArray: any = JSON.parse(
			localStorage.getItem('compareAllOption') || '[]'
		);
		compareAllArray[Obj.id - 1] != undefined
			? (this.treatmentOptionObj = compareAllArray[Obj.id - 1])
			: (this.treatmentOptionObj =
					this.treatmentOption.treatmentOptionsArray);
		this.treatmentOption.treatmentOptionObj = this.treatmentOptionObj;
	}

	addMoreOption() {
		this.menuItems = JSON.parse(
			localStorage.getItem('menuItemArray') || '[]'
		);
	...
	deleteOptions(Obj: any) {
		...
		if (compareAllArray[Obj.id - 1]) {
			this.deleteOptionsValue(
				compareAllArray[Obj.id - 1],
				compareAllArray
			);
	...
	deleteOptionsValue(Obj: any, compareAllArray: any[]) {
		...
		localStorage.setItem(
			'compareAllOption',
			JSON.stringify(compareAllArray)
		);
	}
