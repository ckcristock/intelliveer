...
export class RecommendationsComponent implements OnInit {
	...
	constructor(
		...
		public treatmentOption: ConsultationDiagnosisiProblemListService
	) {}
	...
	ngOnInit(): void {
		...
		this.treatmentOption.treatmentOptionsArray.Treament_Phase =
			this.treamentPhaseOption;
		this.treamentTypeOption = 'Braces';
		this.treatmentOption.treatmentOptionsArray.Treament_Type =
			this.treamentTypeOption;
	}
	...
	ngAfterContentChecked(): void {
		if (this.treatmentOption.treatmentOptionObj.Treament_Time) {
			...
			this.treamentPhaseOption =
				this.treatmentOption.treatmentOptionObj.Treament_Phase;
			this.treamentTypeOption =
				this.treatmentOption.treatmentOptionObj.Treament_Type;
			this.fromTime =
				this.treatmentOption.treatmentOptionObj.Treament_Time.split(
					'-'
				)[0].trim();
			this.toTime =
				this.treatmentOption.treatmentOptionObj.Treament_Time.split(
	...
	moveToNext() {
		let treatmentVisitedArray: any = JSON.parse(
			localStorage.getItem('treatmentVisitedArray') || '[]'
		);
		treatmentVisitedArray.push('Tx Recommendations');
		localStorage.setItem(
			'treatmentVisitedArray',
			JSON.stringify(treatmentVisitedArray)
		);
		this.router.navigate([
			'/dashboard/patient/consultation/consultation/add/treatment/prerequisites'
		]);
	}
	...
	treatmentTime() {
		if (this.fromTime && this.toTime) {
			this.treatmentOption.treatmentOptionsArray.Treament_Time =
				this.fromTime + ' - ' + this.toTime + ' Months';
		}
	}
}
