...
import {
	...
	Section,
	Permissions
} from 'src/app/interfaces/settings/role-management/roles-template.model.';
...
export class AddRoleTemplateComponent implements OnInit {
	...
	get f() {
		return this.roleTemplateForm.controls;
	}
	...
	saveRoleTemplate() {
		...
		if (
			...
			this.roleTemplateForm.value.businessGroups == null ||
			this.roleTemplateForm.value.businessGroups == undefined
		) {
			this.roleTemplateForm.value.businessGroups = [];
		}
	...
	addRoleTemplateForm() {
		this.alertService
			...
			.then((result: any) => {
				if (result.value) {
					this.rolesUserServ
						.createRoleTemplate(this.roleTemplateForm.value)
						.subscribe(
							(res) => {
								...
								this.router.navigate([
									'/dashboard/settings/role-management/manage-role-template'
								]);
							},
							(error) => {
								console.log(error);
	...
	editRoleTemplateForm() {
		...
		this.alertService
			...
			.then((result: any) => {
				if (result.value) {
					this.rolesUserServ
						.updateRoleTemplate(data, this.getRolesID)
						.subscribe(
							(res) => {
								this.alertService.success(
									'Success',
		...
		if (this.getRolesID) {
			permissions.forEach((per: any, i: any) => {
				per.sections.forEach((element: any, index: any) => {
					let resultArray = element.permissions.filter((s: any) =>
						s.name
							.toString()
							.toLowerCase()
	...
	getAllPermissionsTemplateID(data: any, permissionsObj?: any) {
		...
		data.forEach((section: any) => {
			...
			section.permissions.forEach((element: any) => {
				...
				element.permissions.forEach((perm: any) => {
					...
					const permOBJ = permissionsObj.find(
						(name: any) => name.name == perm.name
					);
					if (permOBJ) {
						formGroupSecond.patchValue({
							name: permOBJ.name,
							enabled: permOBJ.enabled,
							locked: permOBJ.locked,
							allowOverride: permOBJ.allowOverride
						});
					} else {
						formGroupSecond.patchValue({
							name: perm.name,
		...
		localStorage.setItem(
			'permissions',
			JSON.stringify(this.roleTemplateForm.value.permissions)
		);
	}
